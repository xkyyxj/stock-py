...
pub async fn calculate_max_win_s(mut conn: PoolConnection<MySql>,
                              stock_codes: Vec<String>, _tx: Sender<u32>,
                              _code2name_map: HashMap<String, String>) {
    let _curr_date_str = time_utils::curr_date_str("%Y%m%d");
    ...
    for item in stock_codes {
        ...
        // 计算周期长度（只计算交易日）
        let _cal_period_num = all_vos.len() - start_index;
        let last_close = all_vos[all_vos.len() - 1].close;
        let start_close = all_vos[start_index].close;
        let _win_pct = (last_close - start_close) / start_close;
